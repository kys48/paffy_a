...
if (!ext.paffy) {
	...
	ext.paffy.Clip = function() {
		var HOST_URL = "http://203.228.244.59";
...
*/
		...
		function getPrice() {
			...
			AF.ajax({
				url : "http://203.228.244.59/scoopsave/is_auto_price",
				type : "get",
				async : false,
				data : {
					name : V
		...
		function r() {
			...
			AG += "#paffy_wrap_layer{z-index:999999;position:absolute;left:0;top:0;width:100%;height:100%;}";
			AG += "#paffy_wrap{z-index:999999;position:absolute;left:0;top:0;width:100%;height:100%;}";
			...
			AG += "#clip_header .clip_logo{width:120px;height:30px;margin:5px auto 0;padding:0;background:url("+HOST_URL+"/images/common/logo.png) no-repeat ;font-size:0;line-height:0;}";
			AG += "#clip_header .b_clipccl{position:absolute;top:8px;right:15px;width:26px;height:25px;background:url("+HOST_URL+"/images/common/clip_close.png) no-repeat;}";
			AG += "#clip_header .b_clipccl:hover{background:url("+HOST_URL+"/images/common/clip_close_on.png) no-repeat;}";
			...
			AG += "#container_clip .cliplist a strong{display:none;position:absolute;left:0px;top:0px;width:215px;height:215px;background:url("+HOST_URL+"/images/common/logo_clip.png) no-repeat;}";
			...
			AG += "#paffy_wrap .layer{position:absolute;font-size:12px;width:700px;z-index:999999;background-color:#FFFFFF;}";
			...
			AG += "#paffy_wrap .layer .close a{display:block;width:26px;height:25px;background:url("+HOST_URL+"/images/common/clip_close.png) no-repeat;}";
		...
		function OpenItemList() {
			...
			var AH = '<div id="paffy_wrap" style="height:' + GetHeight() + 'px;">';
			AH += '<div class="dimmed"></div>';
			AH += '<div id="clip">';
			AH += '<div id="clip_header">';
			AH += '<h1 class="clip_logo"><span class="blind">PAFFY</span></h1>';
			AH += '<a href="javascript:parent.ext.paffy.ClipClass.onClickCloseBtn();" class="bn b_clipccl"><span class="blind">cancel</span></a>';
			AH += "</div>";
			...
			if (J != true) {
				...
				for (var AF = 0; AF < AG; AF++) {
					AH += '<a href="javascript:parent.ext.paffy.ClipClass.submitData(' + AF + ');" onmouseover="parent.ext.paffy.ClipClass.onOverItem(this);"'
			...
			if (J == true) {
				...
				AH += '<td>';
				AH += '<p class="tx">';
				AH += '<img src="http://203.228.244.59/img/alert_scoop1.gif" />';
				AH += '</p>';
				AH += '</td>';
				AH += '</tr>';
				AH += '</table>';
				AH += '</dd>';
				AH += '<dd class="btn">';
				AH += '<button class="bn2 b_com"  onclick="ext.paffy.ClipClass.CloseBlockLayer();"><span class="blind">ok</span></button>';
				AH += '</dd>';
				AH += '</dl>';
				AH += '<div class="layer_fn"></div>';
				AH += '<div class="close"><a href="javascript:parent.ext.paffy.ClipClass.onClickCloseBtn();" class="bn"><span class="blind">close</span></a></div>';
				AH += '</div>';
				...
				if (AG == 0) {
					...
					AH += '</dd>';
					AH += '<dd class="btn">';
					AH += '<button class="bn2 b_clip" onclick="parent.ext.paffy.ClipClass.CloseNoImgLayer();" ><span class="blind">request clip</span></button>';
					AH += '</dd>';
					AH += '</dl>';
					AH += '<div class="layer_fn"></div>';
					AH += '<div class="close"><a href="javascript:parent.ext.paffy.ClipClass.onClickCloseBtn();" class="bn"><span class="blind">close</span></a></div>';
		...
		function OpenPost(AF) {
			...
			var titleStr = document.getElementsByTagName("title")[0].innerHTML;
			
			var AG = CLIP_URL + "?domain=" + V;
			AG += "&url=" + encodeURIComponent(S);
			AG += "&imgurl=" + encodeURIComponent(U[AF]);
			AG += "&price=" + encodeURIComponent(p);
			AG += "&subject=" + encodeURIComponent(H);
			//AG += "&ex_price=" + encodeURIComponent(f);
			//AG += "&exchange=" + encodeURIComponent(R);
			...
			+ '<div class="close">'
			+ '	<a href="javascript:parent.ext.paffy.ClipClass.onClickCloseBtn();" class="bn"><span class="blind">close</span></a>'
			+ '</div>';
