(function() {
	var Conf = function() {
		...
		try {
			...
			for (var i = scripts.length - 1; i >= 0; i--) {
				...
				if (src && /polyvore/.test(src.match(/\/\/([^\/]*)/)[1])) {
	...
	function onClipperReady() {
		window.location = NATIVE_APP_PROTOCOL + "//onClipperReady"
	}

	function onClipperDismissed(complete) {
		window.location = NATIVE_APP_PROTOCOL + "//onClipperDismissed?complete=" + complete
	}
