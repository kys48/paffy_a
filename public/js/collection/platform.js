...
alert(imgPointer.src)
    ...
    self.add = function(element, position, target){
        ...
        var itemAPI    = element.find('input[name^="itemAPI"]').val();
        var itemURL    = element.find('input[name^="itemURL"]').val();
        ...
        if(itemOrigin === 'product'){
            ...
            imgPointer.src = self.embellishmentURL+self.productStyleDirectory+itemImg;
        }else if( $.inArray(itemOrigin, ['set', 'set_draft', 'template_create', 'template_create_draft', 'template']) != -1){
            self.ajaxLoadData(itemId, itemOrigin, itemImg, itemAPI, itemURL);
            return;
        }
        
        imgPointer.onload = function(){
            self.addPreoloadedImage(imgPointer, itemId, itemOrigin, itemImg, itemAPI, itemURL, position, target);  
        }
...
alert('addImageOnInit');    
        ...
        imgPointer.onload = function(){
            ...
            self.addPreoloadedImageOnInit(imgPointer, initData.itemId, initData.itemOrigin, initData.itemImg, initData.itemAPI, initData.itemURL, initData, target);  
        }
