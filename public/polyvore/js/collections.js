function collection(platformType){
        ...
        self.board.sortable({
            ...
            stop: function(event, ui) {
                if(ui.item.hasClass('ui-draggable')){
                    ...
                    ui.item.replaceWith(self.create(itemId,origin));
                }
    ...
    self.create = function(itemId,origin,caption){
        
        var imageSrc;
        
        if(origin === 'product')
            imageSrc = self.productURL+itemId+'.png';
        else if(origin === 'set')
            imageSrc = self.setURL+itemId+'.png';
        else if(origin === 'collection' || origin == 'draft')
            self.ajaxLoadCollection(itemId, origin);
    ...
    self.ajaxLoadCollection = function(collectionId, origin){
    ...
    self.addItems = function(items){
        ...
        $.each(items, function(index, value){
            ...
            var itemHtml = self.create(itemId, itemOrigin, caption);
